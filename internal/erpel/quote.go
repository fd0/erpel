...
func unquoteString(s string) (string, error) {
	...
	if len(s) == 1 {
		return "", errors.WithMessage(fmt.Errorf("invalid quoted string %q", s), s)
	}
...
func unquoteList(s string) (list []string, err error) {
	if len(s) < 2 {
		return nil, errors.WithStack(fmt.Errorf("string %q is too short for a list", s))
	}
	...
	if s[first] != '[' || s[last] != ']' {
		return nil, errors.WithStack(fmt.Errorf("string %q is not a list", s))
	}
